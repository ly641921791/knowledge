...
1. 避免NULL值，很难优化且占用额外索引空间，建议使用0代替NULL。当所在列是唯一索引时，个人一般使用id代替NULL

2. 合理使用[数字类型](../mysql/data-type.md#整数类型)，非负数使用UNSIGNED

3. 合理使用[枚举类型](../mysql/data-type.md#枚举类型)和数字类型代替字符串类型
...
IP（如：127.0.0.1）是由4个8位二进制数字组成，连接在一起是32位二进制数字，转换为十进制数字
...
1. WHERE中避免对字段进行NULL判断，会放弃索引
2. 避免LIKE '%xxx'式查询

1. 使用LIMIT对查询结果的记录进行限定，例如：根据手机号查询用户，使用LIMIT 1限制，避免查询到结果后进行没必要的全表扫描
