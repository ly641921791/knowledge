...
通知/等待相关方法：下面的方法继承自Object类，因为通知/等待都是通过锁完成的，Java的锁是对象级的，
必须使用synchronized代码块的锁对象执行，因为调用前需要获得对象的锁，避免wait和notify之间产生竞态条件。
...
JDK1.0提供了下面三个方法都是过时方法，作用分别是暂停、恢复、停止，由于存在bug已过时。
...
阻塞：线程由于等待某些资源，被挂起。如：ServerSocket#accept()
